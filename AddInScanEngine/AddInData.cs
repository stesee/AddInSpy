...
namespace AddInSpy
{
    public class AddInData
    {
        ...
        internal bool GetVstoAddInData(bool scanManagedInterfaces, bool scanRemote, ref bool isUncPath, ref bool isHttpPath)
        {
            ...
            string sanitizedAssemblyPath;
            if (TryGetAssemblyPath(this.assemblyPath, out sanitizedAssemblyPath))
            {
                this.assemblyPath = sanitizedAssemblyPath;
                assemblyDetails = AssemblyScanner.GetAssemblyInfo(sanitizedAssemblyPath, this.hostName, true);
            }
        ...
        private bool TryGetAssemblyPath(string assemblyPathFromManifest, out string saneAssbmblyPath)
        {
            saneAssbmblyPath = null;

            if (File.Exists(this.assemblyPath))
            {
                saneAssbmblyPath = assemblyPathFromManifest;
                return true;
            }

            if (this.assemblyPath.StartsWith("file:"))
            {
                var localPath = new Uri(assemblyPathFromManifest).LocalPath;

                if (File.Exists(localPath))
                {
                    saneAssbmblyPath = localPath;
                    return true;
                }
            }
